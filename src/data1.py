...
def find_last_peak_and_plot_8002():
    ...
    for i in getpeaktime1:
        ...
        for idx in range(-p8002_1, p8002_2):
            sect_A.append(dataframe.iloc[idx + i, 3] - dataframe.iloc[idx + i + 1, 3])
            sect_B.append(dataframe.iloc[idx + i, 3])
        ...
        lresult2 = []
        for idx in range(-p8002_1, p8002_2):
            if (dataframe.iloc[i + idx, 3] - dataframe.iloc[i + idx + 2, 3]) > 0.01:
                print(str(idx) + "). --------------N:8002_1--------->>> ", dataframe.iloc[i + idx, 0], " <=> ",
                      dataframe.iloc[i + idx, 3] - dataframe.iloc[i + idx + 2, 3])
                dic1[str(dataframe.iloc[i + idx, 0])] = dataframe.iloc[i + idx, 3] - dataframe.iloc[i + 2 + idx, 3]

        for idx in range(-p8002_1, p8002_3):
            if (dataframe.iloc[i + idx, 3] - dataframe.iloc[i + 2 + idx, 3]) > 0.01:
                dic2[str(dataframe.iloc[i + idx, 0])] = dataframe.iloc[i + idx, 3] - dataframe.iloc[i + 2 + idx, 3]
        ...
        sect_B1 = []
        for idx in range(-p8002_1, p8002_2):
            sect_A1.append(dataframe.iloc[idx + i, 4] - dataframe.iloc[idx + i + 1, 4])
            sect_B1.append(dataframe.iloc[idx + i, 4])
        ...
        for idx in range(-p8002_1, p8002_2):
            if (dataframe.iloc[i + idx, 4] - dataframe.iloc[i + 2 + idx, 4]) > 0.01:
                print(str(idx) + "). --------------S:8002_7--------->>> ", dataframe.iloc[i + idx, 0], " <=> ",
                      dataframe.iloc[i + idx, 4] - dataframe.iloc[i + 2 + idx, 4])
                dic3[str(dataframe.iloc[i + idx, 0])] = dataframe.iloc[i + idx, 4] - dataframe.iloc[i + 2 + idx, 4]

        for idx in range(-p8002_1, p8002_3):
            if (dataframe.iloc[i + idx, 4] - dataframe.iloc[i + 2 + idx, 4]) > 0.01:
                dic4[str(dataframe.iloc[i + idx, 0])] = dataframe.iloc[i + idx, 4] - dataframe.iloc[i + 2 + idx, 4]
        ...
        if len(lresult2) > 1 and len(lresult3) > 1:
            axes[0, 0].set_title("XY8002PI==> " + all_file_list[0].split(".")[0].split("_")[1] + "==> 洩漏.發生時間: " + str(
...
def find_last_peak_and_plot_8004():
    ...
    for i in getpeaktime2:
        ...
        lresult2 = []
        for idx in range(-p8004_1, p8004_2):
            if (dataframe.iloc[i + idx, 5] - dataframe.iloc[i + idx + 2, 5]) > 0.01:
        ...
        for idx in range(-p8004_1, p8004_3):
            if (dataframe.iloc[i + idx, 5] - dataframe.iloc[i + 2 + idx, 5]) > 0.01:
        ...
        for idx in range(-p8004_1, p8004_2):
            if (dataframe.iloc[i + idx, 6] - dataframe.iloc[i + 2 + idx, 6]) > 0.01:
